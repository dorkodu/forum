...
export const createDiscussionStore = (props?: Partial<State>) => {
  return createStore(immer<State & Action>((set, get) => ({
    ...initialState,
    ...props,
    ...
    queryGetArguments: async (discussionId, type, refresh) => {
      ...
      if (refresh) discussionStore().setState(state => { discussionId && delete state.discussion.arguments[discussionId] });
      if (_arguments) get().setArguments(discussionId, _arguments, type);
      userStore().setState((store) => {
    ...
    queryGetComments: async (discussionId, type, refresh) => {
      ...
      if (refresh) discussionStore().setState(state => { discussionId && delete state.discussion.comments[discussionId] });
...
type DiscussionProviderProps = React.PropsWithChildren<Partial<State>>

const DiscussionContext = createContext<DiscussionStore | null>(null);
let store: DiscussionStore | undefined = undefined;

export function DiscussionProvider({ children, ...props }: DiscussionProviderProps) {
  if (!store) store = createDiscussionStore(props);

  return (
    <DiscussionContext.Provider value={store}>
      {children}
...
export function useDiscussionStore<T>(
  ...
  const store = useContext(DiscussionContext);
  if (!store) throw new Error('Missing DiscussionContext.Provider in the tree');
  return useStore(store, selector, equalityFn);
}

export function discussionStore() {
  if (!store) throw new Error('Missing DiscussionContext.Provider in the tree');
  return store;
}